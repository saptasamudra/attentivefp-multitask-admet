...
TPE sampler, 20 trials per dataset, remaining 5 datasets only
"""
...
def make_objective(ds_key, ds_cfg):
    def objective(trial):
        ...
        if ds_cfg["task_type"] == "reg":
            return m.get("rmse", 999)
        else:
            return -m.get("roc_auc", 0)
    return objective

# ── MAIN ─────────────────────────────────────────────────────────────────────
def main():
    timestamp = datetime.now().strftime("%Y%m%d_%H%M")
    out_path  = f"results/optuna_moe_remaining_{timestamp}.json"
    all_results = {}
