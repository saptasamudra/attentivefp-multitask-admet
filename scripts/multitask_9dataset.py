"""
Multi-Task AttentiveFP -- 9 Datasets, NO MoE
=============================================
This is the clean baseline to compare against attentivefp_moe.py.
Same 9 datasets, same scaffold split, same seeds, same hyperparameters.
Only difference: no MoE module. Shared encoder -> direct task heads.

Purpose: ablation row "Multi-task (no MoE)" in your paper table.

Run AFTER attentivefp_moe.py finishes:
  python multitask_9dataset.py

Time: ~5-6 hours (no MoE overhead, but HIV adds time)

GenFeatures copied exactly from multitask_7dataset.py (39 atom, 10 bond features)
"""
...
# -----------------------------------------------------------------------------
# CONFIG -- identical to attentivefp_moe.py
# -----------------------------------------------------------------------------
...
# -----------------------------------------------------------------------------
# FEATURE ENGINEERING -- exact copy from multitask_7dataset.py
# produces x=39, edge_attr=10
# -----------------------------------------------------------------------------
...
# -----------------------------------------------------------------------------
# SCAFFOLD SPLIT -- exact copy from multitask_7dataset.py
# -----------------------------------------------------------------------------
...
# -----------------------------------------------------------------------------
# MODEL -- shared encoder + task heads, NO MoE
# -----------------------------------------------------------------------------

class AttentiveFPMultiTask9(nn.Module):
    """
    Plain multi-task AttentiveFP with 9 dataset heads.
    No MoE. This is the direct baseline for the ablation table.
    ...
    def __init__(self, in_channels, edge_dim):
        ...
        # Shared encoder -- identical config to attentivefp_moe.py
        ...
        # Task heads -- 200-dim input (no fusion, no MoE)
        self.heads = nn.ModuleDict({
            name: nn.Linear(h, cfg['num_tasks'])
    ...
    def forward(self, x, edge_index, edge_attr, batch):
        ...
        preds    = {name: head(mol_repr) for name, head in self.heads.items()}
        return preds
...
def run_seed(seed):
    ...
    model = AttentiveFPMultiTask9(in_ch, edge_dim).to(device)
...
if __name__ == '__main__':
    print("=" * 70)
    print("  Multi-Task AttentiveFP (NO MoE) -- 9 Dataset Baseline")
    print(f"  {len(DATASETS)} datasets | No MoE | Direct task heads")
    print("  Purpose: ablation baseline vs attentivefp_moe.py")
    print("=" * 70)
    ...
    print("\n" + "=" * 70)
    print("  MULTI-TASK (NO MoE) RESULTS -- scaffold split -- 3 seeds")
    print("=" * 70)
    ...
    print("=" * 70)
    print("\n  Compare these results against attentivefp_moe.py for ablation table.")
    print("  Key question: does MoE improve over plain multi-task on same 9 datasets?")
