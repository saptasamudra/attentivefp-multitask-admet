...
N_TRIALS   = 30
N_SEEDS    = 5        # match your MoE-GCN protocol (5 seeds)
EPOCHS     = 100
PATIENCE   = 15
BATCH_SIZE = 64
DATA_ROOT  = "./data"
...
# Run all three regression datasets fresh with true DMPNN
DATASETS = [
    {"name": "ESOL"},
    {"name": "FreeSolv"},
    {"name": "Lipo"},
...
print(f"Backbone: TRUE D-MPNN (NNConv + GRUCell)")
...
class DMPNN(nn.Module):
    ...
    """
    def __init__(self, in_ch, edge_ch, hidden, num_layers, dropout):
        super().__init__()
...
def make_objective(train_data, val_data, in_dim, edge_dim):
    def objective(trial):
        ...
        model = DMPNN(in_dim, edge_dim, hidden, num_layers, dropout).to(DEVICE)
...
# Load existing — but re-run all three to get true DMPNN numbers
if os.path.exists(SAVE_PATH):
    with open(SAVE_PATH) as f:
        results = json.load(f)
    # Only skip if already run with true DMPNN
    results = {
...
for ds in DATASETS:
    name = ds["name"]
    if name in results:
        print(f"  Skipping {name} (true DMPNN already done: {results[name]['mean']:.4f})")
        continue

    print(f"\n{'='*60}")
    print(f"  TRUE D-MPNN | {name} | RMSE")
    print(f"{'='*60}")
    t0 = time.time()
    ...
    results[name] = {
        ...
        "backbone":    "true_dmpnn_nnconv_grucell",   # documents the fix
    }
...
print(f"\n{'='*60}")
print("  TRUE D-MPNN REGRESSION COMPLETE")
print(f"{'='*60}")
