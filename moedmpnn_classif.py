...
class MoEDMPNN(nn.Module):
    ...
    def __init__(self, in_ch, edge_ch, hidden, num_layers,
                 dropout, num_experts, top_k, num_tasks):
        super().__init__()
...
def make_objective(train_data, val_data, in_dim, edge_dim, num_tasks):
    def objective(trial):
        ...
        model = MoEDMPNN(in_dim, edge_dim, hidden, num_layers,
                         dropout, num_experts, top_k, num_tasks).to(DEVICE)
...
for ds in DATASETS:
    ...
    print(f"\n{'='*60}")
    print(f"  TRUE MoE-DMPNN | {name} | {num_tasks} tasks | ROC-AUC")
    print(f"{'='*60}")
    t0 = time.time()
...
print(f"\n{'='*60}")
print("  TRUE MoE-DMPNN CLASSIFICATION COMPLETE")
print(f"{'='*60}")
