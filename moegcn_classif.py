"""
MoE-GCN — Classification Benchmark
Datasets: BBBP, BACE, Tox21, ToxCast, SIDER, ClinTox, HIV
Metric: ROC-AUC (higher is better)
Results saved to: results_moegcn_classif.json

Run: python moegcn_classif.py
"""
...
BATCH_SIZE = 64
DATA_ROOT  = "./data"
SAVE_PATH  = "results_moegcn_classif.json"
...
# ── Transform: cast x to float32 at load time ────────────────────────────────
def ToFloat(data):
    data.x = data.x.float()
    return data
...
class MoEGCN(nn.Module):
    ...
    def forward(self, data):
        # x is already float32 due to ToFloat transform
...
def evaluate(model, loader, num_tasks):
    ...
    with torch.no_grad():
        for batch in loader:
            ...
            out, _ = model(batch)                      # [B, num_tasks]
            probs  = torch.sigmoid(out).cpu().numpy()  # [B, num_tasks]
            y      = batch.y.cpu().numpy()             # [B, 1] or [B, T]
            if y.ndim == 1:
                y = y.reshape(-1, num_tasks)
            all_preds.append(probs)
            all_labels.append(y)

    preds  = np.vstack(all_preds)   # [N, num_tasks]
    labels = np.vstack(all_labels)  # [N, num_tasks]
...
def make_objective(train_data, val_data, in_dim, num_tasks):
    def objective(trial):
        ...
        model = MoEGCN(in_dim, hidden, num_layers, dropout, num_experts, top_k, num_tasks).to(DEVICE)
...
if os.path.exists(SAVE_PATH):
    with open(SAVE_PATH) as f:
        results = json.load(f)
    print(f"Resuming — {len(results)} datasets already done")
else:
    results = {}
...
for ds in DATASETS:
    ...
    print(f"\n{'='*55}")
    print(f"  MoE-GCN | {name} | {num_tasks} tasks | ROC-AUC")
    print(f"{'='*55}")
    t0 = time.time()

    # KEY FIX: apply ToFloat transform so x is float32 from the start
    ...
    for seed in range(N_SEEDS):
        ...
        test_auc = evaluate(model, tl, num_tasks)
    ...
    results[name] = {
        ...
        "metric": "roc_auc", "seeds": seed_scores,
...
# ── Summary ───────────────────────────────────────────────────────────────────
print(f"\n{'='*55}")
print("  MoE-GCN CLASSIFICATION SUMMARY")
print(f"{'='*55}")
print(f"{'Dataset':14} {'ROC-AUC':>10} {'±Std':>8}")
print("-" * 35)
for name, r in results.items():
    print(f"{name:14} {r['mean']:>10.4f} {r['std']:>8.4f}")
print(f"\nSaved → {SAVE_PATH}")
